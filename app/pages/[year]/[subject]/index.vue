...
<template>
    ...
    <section class="grid grid-rows-[max-content,1fr] grid-cols-[max-content,1fr] max-lg:gap-2 lg:flex mx-6 lg:mx-10 xl:mx-20 mb-4 lg:mb-6 mt-4 lg:mt-8 items-center gap-3">
        ...
        <ul class="flex flex-wrap lg:ml-auto gap-2 max-lg:col-start-2" v-if="uniqueTags.size > 1">
            <li v-for="tag in uniqueTags" :class="selectedTag.includes(tag) ? 'font-bold text-slate-100 dark:text-slate-900 bg-slate-900/90 dark:bg-slate-50/90 hover:underline' :  'text-slate-700 bg-slate-200/20 hover:bg-slate-200/50 dark:bg-slate-800/20 dark:hover:bg-slate-800/50 hover:text-slate-900 dark:hover:text-slate-50 hover:underline'" @click="() => toggleTags(tag)" class=" dark:text-slate-200 underline-offset-4 rounded-3xl   py-2 px-4 transform-gpu duration-150 cursor-pointer select-none"># {{ tag }}</li>
        </ul>
